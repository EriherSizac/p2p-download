 ...
 *
 * Nota: p2p-files solo necesita TCP (descubrimiento UDP es saliente y no
 * requiere abrir nada). Esta versión es más simple que la del chat, que
 * además abre UDP para WebRTC.
 */
...
export const RULE_NAME = 'p2p-files-inbound';

export function isSupported(): boolean {
  return process.platform === 'win32';
}

/** ¿Existe ya una regla con nuestro nombre apuntando a este puerto? */
export async function ruleExistsForPort(port: number): Promise<boolean> {
  ...
  const script =
    ...
    `if ($p.LocalPort -contains '${port}' -or $p.LocalPort -contains ${port}) { '1' } else { '0' }`;
  try {
 ...
 * Resuelve true si el child exit code == 0.
 */
export function requestFirewallRule(port: number): Promise<boolean> {
  if (!isSupported()) return Promise.resolve(true);

  // Comando que correrá la ventana elevada: idempotente — borra reglas
  // previas con el mismo nombre y crea una nueva para el puerto actual.
  const elevated =
